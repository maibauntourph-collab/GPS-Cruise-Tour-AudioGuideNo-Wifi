...
function enrich() {
    console.log(`--- 🎨 Enriching ${ORSAY_ID} ---`);
    let content = fs.readFileSync(FILE, 'utf-8');

    // Find the landmark object by ID
    const entryStart = content.indexOf(`"id": "${ORSAY_ID}"`);
    if (entryStart === -1) {
        console.error('❌ Orsay ID not found.');
        return;
    }

    // Find the next occurrence of "narration": and "detailedDescription": within this object
    // Note: This matches the outer narration and the ko translation narration
    ...
    if (narrationPos !== -1) {
        ...
        if (valueEnd === -1) valueEnd = content.indexOf('"', valueStart); // last field case

        const original = content.substring(valueStart, valueEnd);
        content = content.substring(0, valueStart) + ENRICHED_NARRATION + content.substring(valueEnd);
        console.log('✅ Main narration enriched.');
    }

    // 2. Korean Translation Narration
    const koNarrationKey = '"ko": {';
    let koPos = content.indexOf(koNarrationKey, entryStart);
    if (koPos !== -1) {
        let koNarrationPos = content.indexOf(narrationKey, koPos);
        if (koNarrationPos !== -1) {
            let valueStart = content.indexOf('"', koNarrationPos + narrationKey.length) + 1;
            let valueEnd = content.indexOf('"', valueStart); // assume it ends with "

            content = content.substring(0, valueStart) + ENRICHED_NARRATION + content.substring(valueEnd);
            console.log('✅ Korean translation enriched.');
        }
    }
    ...
    console.log('🎉 Orsay Enrichment Complete!');
}

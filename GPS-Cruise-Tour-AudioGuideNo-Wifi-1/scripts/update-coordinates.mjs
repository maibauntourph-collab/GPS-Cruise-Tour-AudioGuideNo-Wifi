...
async function updateCoordinates() {
  ...
  for (const [landmarkId, coords] of Object.entries(CORRECT_COORDINATES)) {
    // Use a more specific pattern to match the landmark block
    const escapedId = landmarkId.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
    const pattern = new RegExp(
      `(id:\\s*['"]${escapedId}['"][\\s\\S]*?lat:\\s*)([\\d.-]+)([\\s\\S]*?lng:\\s*)([\\d.-]+)`,
      'g'
    );
    
    const originalContent = content;
    content = content.replace(pattern, `$1${coords.lat}$3${coords.lng}`);
    
    if (content !== originalContent) {
      updatedCount++;
      console.log(`Updated ${landmarkId}: (${coords.lat}, ${coords.lng})`);
    }
  }
